...
static void expand(u32int new_size, heap_t *heap)
{
    ...
    while (i < new_size)
    {
        paging_frame_alloc( paging_page_get(heap->start_address+i, 1, kernel_directory),
...
static u32int contract(u32int new_size, heap_t *heap)
{
    ...
    while (new_size < i)
    {
        paging_frame_free(paging_page_get(heap->start_address+i, 0, kernel_directory));
        i -= 0x1000;
    }
...
heap_t* heap_create(u32int start, u32int end_addr, u32int max, u8int supervisor,
					u8int readonly)
{
